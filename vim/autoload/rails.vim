...
function! s:app_calculate_file_type(path) dict
  ...
  elseif f =~ '_helper\.rb$'
    let r = "helper"
  elseif f =~ '\<app/presenters\>'
    let r = "presenter"
  elseif f =~ '\<app/services\>'
    let r = "service"
...
function! s:AlternateFile()
  ...
  elseif fnamemodify(f,":e") == "rb"
    ...
    elseif t =~ '^presenter\>'
      return s:sub(file,'<app/presenters/','test/unit/')."\n".s:sub(s:sub(file,'_test\.rb$','_spec.rb'),'app/presenters/','spec/presenters/')
    elseif t =~ '^service\>'
      return s:sub(file,'<app/services/','test/unit/')."\n".s:sub(s:sub(file,'_test\.rb$','_spec.rb'),'app/services/','spec/services/')
    elseif t =~ '^test-unit\>'
