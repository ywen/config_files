...
elseif !exists('myprojects_enable')
	...
	function s:myProjects(filename)
		...
		if s:goToWindow() < 0
			...
			nnoremap <buffer> <silent> <C-S-c> :call <SID>setCd(line('.'))<CR>
			nnoremap <buffer> <silent> <C-S-f> :call <SID>setFilter(line('.'), 1)<CR>
		...
		for inode in inodes
			if inode != '.' && inode != '..' && (a:filter == '' || match(inode, a:filter) != -1)
				if isdirectory(inode)
					...
					let files = s:buildFolder(inode, a:level + 1, path, 0, cd, cd != '', filter, filter != a:filter, mappings, a:folderAttributes)
	...
	function s:refresh(line)
		...
		if line > 0
			...
			silent execute ':' . line . ',' . s:getLastFolderLine(line) . 'd'
	...
	function s:appendFolder(folder, line)
		if a:folder != '' && s:goToLine(a:line - 1)
			put =a:folder
			if s:goToLine(a:line)
				normal! zx
			endif
		endif
	endfunction
	...
	function s:setCd(line)
		let cdLine = s:isFolder(a:line) ? a:line : s:getFirstFolderLine(a:line)

		if cdLine > 0 && !s:hasCd(cdLine)
	...
	function s:getFolderAttributes(line)
		...
		if line > 0
			...
			while lineAttribute
				...
				if s:hasPath(lineAttribute)
					let attributes['path'] = s:extractPathFromLine(lineAttribute)
				endif

				if s:hasCd(lineAttribute)
					let attributes['cd'] = s:extractCdFromLine(lineAttribute)
				endif
